...
SG21 has been reviewing a large number of potential use cases for contracts.
There are also a number of potential proposals that have previously been put
forward with might be applicapable to those use cases (see [P2032](http://wg21.link/P2032R0) ).

We (the authors of this paper, acting inependently)
...
of regularly retreading ground while comparing apples to oranges.)

An additional hope is that, as we get proposals that meet this minimal set of
...
Here we will list those use cases that we think should be included, with their
original high-level description (see [P1995](http://wg21.link/P1995R0) for all
of the use cases).
...
<%  
  ...
  for udict in all_data:
      if "x2114.include" in udict:
          ival = int(udict["x2114.include"])
      else:
          ival = 0
      if ival:
          displaydata.append(udict)
%>
  
% for udict in displaydata:
  <tr>
  <td>${n}</td>
  <td>${udict["label"]}</td>
  <td>${udict["asa"]}</td>
...
% if "x2114.notes" in udict:
<tr>
  <td colspan="9"> ${formatmarkdown(udict["x2114.notes"])} </td>
</tr>
% endif
...
This began as an excercise amongst the authors, and we hope that we have identified
something that would be minimally viable to agree on as a group.

We encourage anyone to compare these results with the use cases and convince
